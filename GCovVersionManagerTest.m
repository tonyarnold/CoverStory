...
- (void)testVersionCheck
{
    ...
    NSDictionary *testRecordData = @{ @"test_i386_4_0.gcda" : @"4.0",
    ...
    [testRecordData enumerateKeysAndObjectsUsingBlock:^(NSString *name, NSString *version, BOOL *stop) {
        NSString *path = [testBundle pathForResource:name ofType:nil];
        XCTAssertEqualObjects([mgr versionFromGCovFile:path], version, @"%@ (version %@) - path: %@", name, version, path);
    }];
